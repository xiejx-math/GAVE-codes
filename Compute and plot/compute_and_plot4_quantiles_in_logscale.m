...
%   method
% This code is adapted the code structure of an randomized sparse
% Kaczmarz method which is coded by Dirk A. Lorenz and Maximilian Winkler
% https://github.com/MaxiWk/Minimal-error-momentum-Bregman-Kaczmarz
%
% Modifed by Jiaxin Xie  Email: xiejx@buaa.edu.cn


function [x_arrays, quantiles] = compute_and_plot4_quantiles_in_logscale(x_arrays, arrs, ...
...
for i = 1:num_methods
    ...
    if i==1
        plot_array(i) = plot( x_arrays{i}, log10(medians{i}), line_colors{i}, 'LineWidth', 1.2,...
            'LineStyle', '-', 'Marker', 'o','DisplayName', display_names{i});
    else if i==2
            plot_array(i) = plot( x_arrays{i}, log10(medians{i}), line_colors{i}, 'LineWidth', 1.2,...
                'LineStyle', '-' ,'Marker', 's','DisplayName', display_names{i});
    else if i==3
            plot_array(i) = plot( x_arrays{i}, log10(medians{i}), line_colors{i}, 'LineWidth', 1.2,...
                'LineStyle', '-','Marker', '^', 'DisplayName', display_names{i});
    else if i==4
        plot_array(i) = plot( x_arrays{i}, log10(medians{i}), line_colors{i}, 'LineWidth', 1.2,...
            'LineStyle', '-', 'Marker', 'x','DisplayName', display_names{i});
    end
    end
    end
    end
...
if display_legend
    legend(plot_array, display_names, 'location', 'best');
end
